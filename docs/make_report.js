...
// real example charts + equity curve (generated by build_charts.js)
let CHARTS = { examples: [], equity: { svg: '', totalR: 0, nTrades: 0, years: [] } };
try { CHARTS = JSON.parse(fs.readFileSync(SCR + '/charts.json', 'utf8')); } catch (e) {}
const REASON = { TP3: 'alle Ziele erreicht', TRAIL_SL: 'Trailing-Stop', BE_SL: 'Break-Even-Stop', SL: 'Stop-Loss', TIMEOUT: 'Zeit-Exit' };
function examplesHTML() {
  return CHARTS.examples.map((e, i) => {
    const win = e.r >= 0;
    return `<div class="fig">${e.svg}<div class="figcap">Abb. ${5 + i} — <b>${e.tk}</b>, Entry ${e.entryDate} · ${win ? 'Gewinner' : 'Verlierer'} <b style="color:${win ? '#0b7a43' : '#d1403f'}">${win ? '+' : ''}${e.r.toFixed(1)} R</b> · Exit: ${REASON[e.reason] || e.reason} nach ${e.held} Tagen.</div></div>`;
  }).join('\n');
}
function yearlyRows() {
  return CHARTS.equity.years.map(y => `<tr><td>${y.y}</td><td>${y.r >= 0 ? '+' : ''}${y.r} R</td><td>${y.n}</td><td>${(y.r / y.n).toFixed(2)} R</td></tr>`).join('');
}
...
<h3>Total R über die getradete Periode (2013–2018)</h3>
<p>Kumulierte R-Vielfache aller ${CHARTS.equity.nTrades} Trades (nach Realisierungs­datum). „R" = Vielfaches des Anfangsrisikos; bei 1 % Risiko/Trade entspricht 1 R ≈ 1 % Depot.</p>
<div class="fig">${CHARTS.equity.svg}<div class="figcap">Abb. 4 — Kumulierte R-Kurve über 498 Aktien. Endstand <b>+${CHARTS.equity.totalR} R</b> über die ~5 Jahre.</div></div>
<table>
<tr><th>Jahr</th><th>Total R</th><th>Trades</th><th>Ø R/Trade</th></tr>
${yearlyRows()}
<tr style="font-weight:700"><td>Gesamt</td><td>+${CHARTS.equity.totalR} R</td><td>${CHARTS.equity.nTrades}</td><td>${(CHARTS.equity.totalR / CHARTS.equity.nTrades).toFixed(2)} R</td></tr>
</table>
<div class="box navy"><b>So liest man das:</b> +${CHARTS.equity.totalR} R ist die <i>Summe über alle Aktien</i> — als ob du jedes Signal auf jeder Aktie gehandelt hättest. Ein einzelner Trader nimmt davon einen Bruchteil (z. B. mit 5–8 gleichzeitigen Positionen). Aussagekräftig ist die <b>Konsistenz</b>: jedes volle Jahr positiv, ~0,9 R pro Trade im Schnitt.</div>

<h3 class="pagebreak">Beispiel-Entries — echte Trades aus dem Backtest</h3>
<p>Vier reale Trades mit dem exakten Setup (Kerzen · <span style="color:#e08a1e;font-weight:700">SMA150</span> · <span style="color:#2b6cb0;font-weight:700">SMA50</span> · Entry/SL/TP-Level). So sieht der Stage-1→2-Retest im Chart aus — inklusive eines Verlierers zur Ehrlichkeit.</p>
${examplesHTML()}
...
<h2 class="pagebreak">12 · Alltag: Routine &amp; Checkliste (Vollzeit)</h2>
<div class="box green"><b>Warum das mit Vollzeit funktioniert:</b> Daily-Timeframe → Entscheidungen fallen auf der <i>geschlossenen</i> Tageskerze, und die Einstiege sind <b>Limit-/Stop-Orders, die du vorab platzierst</b>. Sie führen sich während der US-Handelszeit von selbst aus, während du arbeitest — <b>kein Intraday-Screen nötig</b>.</div>
<p><b>Zeitzonen-Realität:</b> Deine US-Aktien handeln <b>15:30–22:00 deutscher Zeit</b>; die Tageskerze ist erst um 22:00 fertig. Deshalb wird <b>morgens</b> (frische Kerze von gestern) und <b>am Wochenende</b> analysiert. Die 20:00-Fenster liegen mitten in der US-Session → zum <b>Planen &amp; Orders setzen</b> nutzen, nicht um auf einer halbfertigen Kerze zu handeln.</p>
<table>
<tr><th>Dein Zeitfenster</th><th>Aufgabe</th></tr>
<tr><td><b>Wochenende (1–2 h)</b> 🎯 Kern-Session</td><td>Markt-Regime prüfen (S&amp;P 500 über steigender 30-Wo-Linie?) · Screen laufen lassen · <b>Watchlist der Woche</b> bauen (Entry/SL/Target fix) · offene Positionen auf dem Wochenchart checken</td></tr>
<tr><td><b>Mo 20:00 (1 h)</b></td><td>Watchlist finalisieren · <b>GTC-Bracket-Orders</b> platzieren · Preis-Alerts setzen</td></tr>
<tr><td><b>Werktags morgens (30 min)</b></td><td>Gestrige Tageskerze checken · <b>Trailing-Stops nachziehen</b> (nur hoch) · Orders anpassen</td></tr>
<tr><td><b>Werktags nach Arbeit (30 min)</b></td><td><i>Optional</i> — Journal, Fills prüfen, morgen vorbereiten. <b>Nicht</b> die laufende Kerze handeln</td></tr>
<tr><td><b>Fr 20:00 (1 h)</b></td><td>Wochen-Review: was hat getriggert, Positionsstatus · Wochenende vorbereiten</td></tr>
</table>
<p><b>Das Wochenende macht ~80 % der Arbeit</b> — werktags ist nur Ausführung. „Nichts zu tun" ist die häufigste und beste Antwort.</p>

<h3>Konkret mit TradingView + IBKR</h3>
<ul>
<li><b>Chart einrichten:</b> Daily-Chart, Indikatoren <b>SMA 150</b>, <b>SMA 50</b> und <b>ATR(14)</b> hinzufügen. Setup = Kurs bricht über 20-Tage-Hoch <i>und</i> über die (flach/steigende) SMA 150, Volumen ≥ 1,5×Ø.</li>
<li><b>Alerts:</b> in TradingView einen Preis-Alert auf das Ausbruchs-/Retest-Level legen → Handy-Push, du musst nicht live dabei sein.</li>
<li><b>Order über IBKR:</b> aus TradingView die <b>Bracket-Order</b> an IBKR routen: Buy-Limit (Retest) + angehängter Sell-Stop (1,5×ATR) + Teilverkauf-Limits (2R/4R), als <b>GTC</b>. Läuft automatisch während der Arbeit.</li>
<li><b>ATR ablesen:</b> aktuellen ATR(14)-Wert nehmen → SL = Entry − 1,5×ATR, R = 1,5×ATR, TP1 = Entry+2R, TP2 = Entry+4R.</li>
</ul>

<h3>Checklisten zum Abhaken</h3>
<div class="grid">
<div class="box navy" style="flex:1"><b>☐ Wochenende</b><br>
☐ Ist der S&amp;P 500 über der steigenden 30-Wo-Linie? (sonst defensiv)<br>
☐ Screen: neue Stage-1→2-Ausbrüche + Stage-2-Pullbacks<br>
☐ Watchlist 5–10 Titel mit Entry/SL/TP notieren<br>
☐ Offene Positionen: noch Stage 2? Nähe Stage 3?</div>
<div class="box navy" style="flex:1"><b>☐ Täglich (30 min)</b><br>
☐ Getriggerte Orders / Fills prüfen<br>
☐ Trailing-Stops auf Basis der Vortageskerze nachziehen (nur hoch)<br>
☐ Watchlist-Level erreicht? Bracket-Order/Alert setzen<br>
☐ Max. 5–8 offene Positionen einhalten</div>
</div>
<div class="box amber"><b>Disziplin-Regeln:</b> Nicht jeden Tag traden (leere Tage sind normal) · Trailing-Stop nur 1×/Tag · alles per Order automatisieren (Emotionen raus) · Position immer per fixem Risiko sizen: <span class="mono">Stück = 1 % Depot ÷ (Entry − SL)</span>.</div>
<p><b>Minimal-Variante:</b> Weinstein ist wochen-orientiert — du kannst rein am Wochenende GTC-Bracket-Orders für die ganze Woche setzen und werktags nur die Trailing-Stops nachziehen. Das reicht.</p>
